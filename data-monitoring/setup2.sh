...
: '
setup_2.0.sh - Data Monitoring & Preprocessing Setup Script

This script automates the setup of data monitoring and preprocessing tools for a specified project in the NDClab environment.

Usage:
  setup.sh [-t] [-f <flags>] <project-name>

  Example:
    ./setup.sh -t -f "--suppress-missing-pending-qa --no-color" my_project

Options:
  -t                Set up a central tracker for the project.
  -f <flags>        Pass extra flags as strings to be set in hallMonitor2.sub.

Main Steps:
  1. Loads necessary modules.
  2. Displays NDCLab Banner.
  3. Validates the project name and existence of the project directory.
  4. Sources lab tools and collects all new REDCap files in the project.
  5. If -t is specified, generates a central tracker CSV using sing_image.

Requirements:
  - NDClab environment with required paths and modules.
  - Project directory must exist in /home/data/NDClab/datasets.
  - hallMonitor2.sub and preprocess.sub templates must be available in labpath/template.

Notes:
  - hallMonitor2.sub is not overwritten if it already exists.
  - TODO: Ensure Hallmonitor2.sub is included in the git repository.
  - TODO: Verify flags passed to hallMonitor2.sub are valid.
'
usage() { echo "Usage: setup.sh [-t] [-f <flags>] <project-name>" 1>&2; exit 1; }
